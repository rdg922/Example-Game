...
const JUMP_FORCE = 800			# Force applied on jumping
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.pressed:
			...
			$Chain.shoot(event.position - position)
		else:
...
func _physics_process(_delta: float) -> void:
	...
	var collision = move_and_slide(velocity, Vector2.UP)	# Actually apply all the forces
	if $Chain.hooked:
		if (last_velocity.y - chain_velocity.y) > 0:
			print("Shake")
		last_velocity = chain_velocity
